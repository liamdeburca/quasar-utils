...
class PlottingInfo(_Info):
    ...
    def update_from_self(self) -> None:
        ...
        if missing("column_sep"):
            self["column_sep"] = max(
                [self["double_w"] - 2 * self["single_w"], 0]
            )
    ...
    @staticmethod
    @validate_call
    def from_file(path: AbsoluteFilePath | None = None):
        ...
        for count, line in enumerate(lines, start=1):
            ...
            if key in rcParams.keys():
                pinfo["rc"][key] = line[1] if len(line[1:]) == 1 else line[1:]

                msg = (
                    f">>> [{count}/{len(lines)}] (rcParams) '{key}': {f(val)}."
                )
