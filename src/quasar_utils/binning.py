...
def log_edges(
    ...
    x_edges     = empty(N+1, dtype=float64)
    x_edges[:N] = x - dx / 2
    x_edges[-1] = x[-1] + dx[-1] / 2

    n_xr = log(x_edges[-1] / x_edges[0]) // log(1 + v_res) + 1
    log_xr_edges = log(x_edges[0]) + log(1 + v_res) * arange(n_xr)
...
def alpha_matrix_elements(
    ...
    """
    Numba-optimized function to compute the non-zero elements of the alpha 
...
def log_resample(
    ...
    *,
...
) -> tuple[FittableFloatVector, FittableFloatVector, FittableFloatVector]:
    """
    ...
    if conserve: alpha_matrix /= alpha_matrix.sum(axis=1)[:,None]
