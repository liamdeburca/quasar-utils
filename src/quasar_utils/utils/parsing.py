...
from numpy import array, inf, nan_to_num, ndarray
...
def get_lines_from_file(
    ...
    with open(path) as f:
        all_lines = [
            line for line in map(trim_line, f.readlines()) if len(line) > 0
        ]
...
def as_float_or_int(s: str) -> float | int | None:
    f = as_float(s)
    if (f is None) or ("." in s):
        return f
    else:
        return int(f)
...
def as_list_of_floats(s: str) -> list[float | None]:
    return _map(as_float, as_list(s))


def as_list_of_ints(s: str) -> list[int | None]:
    return _map(as_int, as_list(s))


def as_list_of_bools(s: str) -> list[bool | None]:
...
def as_scalar_or_quantity(l: list[str]) -> float | int | Quantity:
    if len(l) == 1:
        return as_float_or_int(l[0])
    else:
        return as_quantity(l)
...
def as_list_of_scalars_or_quantity(l: list[str]) -> list | Quantity:
    if len(l) == 1:
        return _map(as_float_or_int, as_list(l[0]))
    else:
        return as_list_of_quantity(l)
...
def as_bounds_of_scalars_or_quantity(l: list[str]) -> tuple | Quantity:
    if len(l) == 2:
        return as_bounds(l)
    else:
        return as_bounds_of_quantity(l)
...
def as_pairs_of_floats_or_quantity(l: list[str]) -> list | Quantity:
    if len(l) == 1:
        return as_pairs_of_floats(l)
    else:
        return as_pairs_of_quantity(l)


### SPECIAL CASES


def as_loading_z(l: list[str]) -> float | tuple:
...
def as_list_of_lines(l: list[str]) -> list[str | float | Quantity]:
    unit = 1.0 if len(l) == 1 else as_unit(l[1])
    return [
        as_float(s) * unit if s.isnumeric() else "all" for s in as_list(l[0])
    ]
