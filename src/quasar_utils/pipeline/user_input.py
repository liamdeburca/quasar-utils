...
def get_yes_or_no(
    ...
    while tries <= max_tries:
        ...
        user_input = input(
            prompt if prefix is None else prefix + "\n" + prompt
        )
        if not user_input:
            user_input = default
        ...
        prefix = f"Please type either 'y', 'n', or the enter key ({default})! [{tries}/{max_tries}]"
...
def get_alternative_dir(
    ...
    while tries <= max_tries:
        if tries == max_tries:
            prefix = (
                "Last try! Will randomise name if failed again!\n" + prefix
            )
