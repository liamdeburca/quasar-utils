...
class TestOutputDirPickle:
    ...
    def test_basic_pickle_and_unpickle(
        self, input_dir_fixture, temp_output_dir
    ):
    ...
    def test_pickle_preserves_input_dir(
        self, input_dir_fixture, temp_output_dir
    ):
    ...
    def test_pickle_preserves_output_path(
        self, input_dir_fixture, temp_output_dir
    ):
    ...
    def test_pickle_preserves_subdirs(
        self, input_dir_fixture, temp_output_dir
    ):
        ...
        for original, unpickled_subdir in zip(
            original_subdirs, unpickled_subdirs
        ):
    ...
    def test_getstate_returns_dict_with_expected_keys(
        self, input_dir_fixture, temp_output_dir
    ):
    ...
    def test_pickle_preserves_set_behavior(
        self, input_dir_fixture, temp_output_dir
    ):
    ...
    def test_pickle_with_dangerous_false(
        self, input_dir_fixture, temp_output_dir
    ):
        """Test pickling OutputDir with dangerous=False (default)."""
        output_dir = OutputDir(
            input_dir_fixture, path=temp_output_dir, dangerous=False
        )
    ...
    def test_pickle_maintains_subdir_attributes(
        self, input_dir_fixture, temp_output_dir
    ):
        ...
        for original_subdir, unpickled_subdir in zip(
            original_subdirs, unpickled_subdirs
        ):
...
class TestOutputDirWithTenAscFiles:
    ...
    def test_create_output_dir_with_ten_asc_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        """Test that OutputDir creates 10 SubDirs for 10 input files."""
        output_dir = OutputDir(
            input_dir_with_ten_asc_files, path=temp_output_dir_for_ten_files
        )
    ...
    def test_output_dir_subdirs_attribute_size(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_set_size_is_ten(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_iteration_yields_ten_subdirs(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_subdirs_point_to_correct_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_with_ten_files_no_duplicates(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_with_ten_files_all_unique_input_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_preserves_input_dir_with_ten_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
...
class TestOutputDirBatched:
    ...
    def test_batched_basic_division(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_size_equals_total(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_creates_subdirs(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_preserves_output_path(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_with_custom_path(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_with_custom_input_dir(
        self, temp_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        ...
        with tempfile.TemporaryDirectory() as custom_tmpdir:
            ...
            batches = list(
                output_dir.batched(batch_size=2, input_dir=custom_input_dir)
            )
    ...
    def test_batched_covers_all_subdirs(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_batched_each_batch_has_correct_input_dir(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
...
class TestOutputDir:
    ...
    def test_output_dir_resets_dangerous(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        output_dir = OutputDir(
            input_dir_with_ten_asc_files, path=temp_output_dir_for_ten_files
    ...
    def test_output_dir_creates_ten_subdirs_for_ten_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        """Test that OutputDir creates 10 SubDirs for 10 input files."""
        output_dir = OutputDir(
        ...
        assert len(output_dir) == 10, (
            f"Expected 10 SubDirs, got {len(output_dir)}"
        )

    def test_output_dir_all_subdirs_have_input_files(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        ...
        input_files = set(input_dir_with_ten_asc_files.files)
        for subdir in output_dir:
            assert hasattr(subdir, "in_file"), (
                "SubDir should have in_file attribute"
            )
            assert subdir.in_file in input_files, (
                f"SubDir in_file {subdir.in_file} not in input files"
            )

    def test_output_dir_single_file_subdir_attributes(
        self, input_dir_fixture, temp_output_dir
    ):
        ...
        assert len(output_dir) == 1, (
            f"Expected 1 SubDir, got {len(output_dir)}"
        )
    ...
    def test_output_dir_does_not_lose_subdirs_on_access(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        ...
        assert first_access == 10, (
            f"First access: expected 10 SubDirs, got {first_access}"
        )
        assert second_access == 10, (
            f"Second access: expected 10 SubDirs, got {second_access}"
        )
        assert third_access == 10, (
            f"Third access: expected 10 SubDirs, got {third_access}"
        )

    def test_output_dir_subdirs_match_input_files_count(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        ...
        assert num_subdirs == num_input_files, (
            f"SubDir count ({num_subdirs}) should match input file count ({num_input_files})"
        )

    def test_output_dir_as_set_has_correct_size(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
    ...
    def test_output_dir_iteration_does_not_modify_size(
        self, input_dir_with_ten_asc_files, temp_output_dir_for_ten_files
    ):
        ...
        assert initial_size == final_size == 10, (
            f"Size changed during iteration: {initial_size} -> {final_size}"
        )
