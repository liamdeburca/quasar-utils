...
def test_cython_vs_numba_no_bias():
    ...
    M_numba, bias_numba = create_interp_matrix_numba(
        x, xb, left=0.0, right=0.0
    )
    M_cython, bias_cython = create_interp_matrix_cython(
        x, xb, left=0.0, right=0.0
    )

    # Verify sparse matrix data is identical
    assert M_numba.shape == M_cython.shape, "Shape mismatch"
    assert np.array_equal(M_numba.data, M_cython.data), (
        "Sparse matrix data mismatch"
    )
    assert np.array_equal(M_numba.indices, M_cython.indices), (
        "Column indices mismatch"
    )
    assert np.array_equal(M_numba.indptr, M_cython.indptr), (
        "Row pointers mismatch"
    )
    ...
    assert np.allclose(M_numba.toarray(), M_cython.toarray()), (
...
def test_cython_vs_numpy_interp_with_extrapolation():
    ...
    # Get Cython matrix result
    M, bias = create_interp_matrix_cython(
        x, xb, left=left_val, right=right_val
    )
    cython_result = M @ values + bias
