...
from quasar_utils.interpolation import (
    create_interp_matrix as create_interp_matrix_cython,
)

# Import implementations
from quasar_utils.interpolation.interpolation import (
    create_interp_matrix as create_interp_matrix_numba,
)


def setup_test_data(
    seed: int = 42, n_in: int = 100, n_out: int = 200
...
def benchmark_numba_compiled(
    x: np.ndarray, xb: np.ndarray, n_runs: int = 100
) -> float:
    """
...
def benchmark_numpy_interp(
    x: np.ndarray, xb: np.ndarray, values: np.ndarray, n_runs: int = 100
) -> float:
    """
...
def calculate_breakeven(
    numba_compilation_time: float,
    numba_per_call: float,
    cython_per_call: float,
) -> tuple[int, float]:
    """
    ...
    if cython_per_call < numba_per_call:
        ...
        return int(
            np.ceil(numba_compilation_time / 0.0001)
        ), numba_compilation_time  # Effectively never
    else:
...
def run_benchmark(n_in: int = 100, n_out: int = 200, n_runs: int = 100):
    ...
    print("=" * 70)
    print("INTERPOLATION BENCHMARK: Cython vs Numba")
    print("=" * 70)
    print("\nTest configuration:")
    ...
    print(
        f"   Total for {n_runs} calls: {numba_exec_time * n_runs * 1000:.1f} ms"
    )
    ...
    print(
        f"   Total for {n_runs} calls: {cython_exec_time * n_runs * 1000:.1f} ms"
    )
    ...
    print(
        f"   Total for {n_runs} calls: {numpy_exec_time * n_runs * 1000:.1f} ms"
    )
    ...
    if speedup_cython_vs_numba > 1.0:
        ...
        print(
            f"  → Numba is {1 / speedup_cython_vs_numba:.2f}x faster per call"
        )
    ...
    if speedup_cython_vs_numpy > 1.0:
        print(
            f"  → Cython is {speedup_cython_vs_numpy:.1f}x SLOWER (expected, different approach)"
        )
        print(
            "  ⚠ Note: Cython builds reusable matrix; NumPy directly interpolates"
        )
    else:
        print(
            f"  → Cython is {1 / speedup_cython_vs_numpy:.1f}x faster per call"
        )

    # Break-even analysis
    breakeven_calls, breakeven_time = calculate_breakeven(
        numba_compile_time, numba_exec_time, cython_exec_time
    )

    print("\nBREAK-EVEN ANALYSIS:")
    if breakeven_calls == 0:
        print("  ✓ Cython is FASTER from the very first call!")
        print("  → No JIT compilation overhead")
        print("  → Per-call execution is faster")
    ...
    print("=" * 70)
    print(
        f"{'Implementation':<25} {'Per-call (ms)':<15} {'100 calls (ms)':<15}"
    )
    print("-" * 70)
    print(
        f"{'NumPy.interp':<25} {numpy_exec_time * 1000:<15.3f} {numpy_exec_time * 100 * 1000:<15.1f}"
    )
    print(
        f"{'Cython':<25} {cython_exec_time * 1000:<15.3f} {cython_exec_time * 100 * 1000:<15.1f}"
    )
    print(
        f"{'Numba (compiled)':<25} {numba_exec_time * 1000:<15.3f} {numba_exec_time * 100 * 1000:<15.1f}"
    )

    print("\nNotes:")
    print("  • NumPy.interp: Direct interpolation (no matrix overhead)")
    print("  • Cython/Numba: Matrix creation for later application")
    print(
        f"  • NumPy is {cython_exec_time / numpy_exec_time:.1f}x faster per call"
    )
    print("  • Matrix approach amortizes cost over multiple interpolations")
    print("  • Use NumPy.interp for: Single or few one-off interpolations")
    print(
        "  • Use Cython for: Reusable matrices (multiple data arrays, same x)"
    )

    if breakeven_calls > 0:
        total_numba_at_breakeven = (
            numba_compile_time + breakeven_calls * numba_exec_time
        )
        print(f"\n{'At break-even point:':<20} {breakeven_calls} calls")
        print(
            f"{'Numba total time':<20} {total_numba_at_breakeven * 1000:<15.1f} ms"
        )
        print(f"{'Cython total time':<20} {breakeven_time * 1000:<15.1f} ms")
    else:
        total_numba_with_compilation = (
            numba_compile_time + numba_exec_time * 100
        )
        total_cython = cython_exec_time * 100
        savings_100_calls = (
            total_numba_with_compilation - total_cython
        ) * 1000
        print("\nComparison (including 100 calls):")
        print(
            f"{'  Numba (1st call + 100 runs):':<35} {total_numba_with_compilation * 1000:.1f} ms"
        )
